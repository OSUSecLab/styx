...
// OCalls
extern "C" {
	sgx_status_t ocall_ecdsa_get_qe_target_info(uint32_t *ret, sgx_target_info_t* qe_target_info);
	sgx_status_t ocall_ecdsa_quote_generation(uint32_t *ret, uint32_t* quote_size, sgx_report_t* app_report, uint8_t* quote_buffer);
	sgx_status_t ocall_ecdsa_quote_verification(uint32_t *ret, uint8_t* quote_buffer, uint32_t quote_size);
}
...
static sgx_status_t dh_dcap_generate_message2(const pcd_dh_dcap_msg1_t *msg1,
                                         const sgx_ec256_public_t *g_b,
                                         const sgx_key_128bit_t *dh_smk,
                                         pcd_dh_dcap_msg2_t *msg2);

template <decltype(dh_dcap_generate_message2) gen_msg2>
static sgx_status_t dh_dcap_initiator_proc_msg1(const pcd_dh_dcap_msg1_t* msg1, pcd_dh_dcap_msg2_t* msg2, sgx_dh_session_t* sgx_dh_session);

static sgx_status_t dh_dcap_verify_message3(const pcd_dh_dcap_msg3_t *msg3,
                                       const sgx_ec256_public_t *g_a,
                                       const sgx_ec256_public_t *g_b,
                                       const sgx_key_128bit_t *dh_smk);

template <decltype(dh_dcap_verify_message3) ver_msg3>
static sgx_status_t dh_dcap_initiator_proc_msg3(const pcd_dh_dcap_msg3_t* msg3,
    sgx_dh_session_t* sgx_dh_session, sgx_key_128bit_t* aek,
    sgx_dh_session_enclave_identity_t* responder_identity);



static sgx_status_t dh_dcap_generate_message1(pcd_dh_dcap_msg1_t *msg1, sgx_internal_dh_session_t *context);

static sgx_status_t dh_dcap_generate_message3(const pcd_dh_dcap_msg2_t *msg2,
                                         const sgx_ec256_public_t *g_a,
                                         const sgx_key_128bit_t *dh_smk,
                                         pcd_dh_dcap_msg3_t *msg3);
...
// sgx_status_t pcd_dh_dcap_init_session()
...
sgx_status_t pcd_dh_dcap_init_session(sgx_dh_session_role_t role, sgx_dh_session_t* sgx_dh_session)
{
...
//pcd_dh_dcap_initiator_proc_msg1 processes M1 message, generates M2 message and makes update to the context of the session.
sgx_status_t pcd_dh_dcap_initiator_proc_msg1(const pcd_dh_dcap_msg1_t* msg1,
    pcd_dh_dcap_msg2_t* msg2, sgx_dh_session_t* sgx_dh_session)
{
...
static sgx_status_t dh_dcap_initiator_proc_msg1(const pcd_dh_dcap_msg1_t* msg1, pcd_dh_dcap_msg2_t* msg2, sgx_dh_session_t* sgx_dh_session)
{
    ...
    if(SGX_SUCCESS != se_ret)
    {
		pcd_log_error("ERROR: dh_dcap_initiator_proc_msg1: sgx_ecc256_open_context returned %d\n", se_ret);
        goto error;
    }
    ...
    if(SGX_SUCCESS != se_ret)
    {
		pcd_log_error("ERROR: dh_dcap_initiator_proc_msg1: sgx_ecc256_create_key_pair returned %d\n", se_ret);
        goto error;
    }
    ...
    if(SGX_SUCCESS != se_ret)
    {
		pcd_log_error("ERROR: dh_dcap_initiator_proc_msg1: sgx_ecc256_compute_shared_dhkey returned %d\n", se_ret);
        goto error;
    }
    ...
    if(SGX_SUCCESS != se_ret)
    {
		pcd_log_error("ERROR: dh_dcap_initiator_proc_msg1: derive_key returned %d\n", se_ret);
        goto error;
    }
    ...
    if(SGX_SUCCESS != se_ret)
    {
		pcd_log_error("ERROR: dh_dcap_initiator_proc_msg1: gen_msg2 returned %d\n", se_ret);
        goto error;
    }
    ...
    if(SGX_SUCCESS != sgx_ecc256_close_context(ecc_state))
    {
        ...
        // set error state
        session->initiator.state = SGX_DH_SESSION_STATE_ERROR;
        pcd_log_error("ERROR: dh_dcap_initiator_proc_msg1: DH session state error\n");
                                         ...
                                         pcd_dh_dcap_msg2_t *msg2)
{
    ...
    memset(msg2, 0, sizeof(pcd_dh_dcap_msg2_t));
    memcpy(&msg2->g_b, g_b, sizeof(sgx_ec256_public_t));

    // Generate challenger report
    maced_size = static_cast<uint32_t>(sizeof(pcd_dh_dcap_msg3_body_t));

    memcpy(msg_buf, g_b, sizeof(sgx_ec256_public_t));
    memcpy(msg_buf + sizeof(sgx_ec256_public_t), &msg1->g_a, sizeof(sgx_ec256_public_t));
    ...
    uint32_t retstatus;
    se_ret = ocall_ecdsa_get_qe_target_info(&retstatus, &qe_target_info);
    if (SGX_SUCCESS == se_ret) {
        if (retstatus != 0)
            return se_ret;
    }
    ...
    uint32_t quote_size;
    se_ret = ocall_ecdsa_quote_generation(&retstatus, &quote_size, &app_report, quote_buffer);
    if (se_ret == SGX_SUCCESS)
    {
        if (retstatus == -1)
            return se_ret;
...
//pcd_dh_dcap_initiator_proc_msg3 processes M3 message, and returns the session key AEK.
sgx_status_t pcd_dh_dcap_initiator_proc_msg3(const pcd_dh_dcap_msg3_t* msg3,
    sgx_dh_session_t* sgx_dh_session, sgx_key_128bit_t* aek,
    sgx_dh_session_enclave_identity_t* responder_identity)
{
    ...
    sgx_dh_session_enclave_identity_t* responder_identity)
{
    ...
    if(SGX_SUCCESS != se_ret)
    {
		pcd_log_error("ERROR: ver_msg3 returned %d\n", se_ret);
        goto error;
    }
    ...
    if(SGX_SUCCESS != se_ret)
    {
		pcd_log_error("ERROR: derive_key returned %d\n", se_ret);
        goto error;
    }
                                       ...
                                       const sgx_key_128bit_t *dh_smk)
{
    ...
    memcpy(quote, &msg3->msg3_body.quote_buffer, sizeof(msg3->msg3_body.quote_buffer));

    // Verify message 3 report
    uint32_t retstatus;
    quote_size = msg3->msg3_body.quote_size;
	se_ret = ocall_ecdsa_quote_verification(&retstatus, quote, quote_size);
    if (se_ret == SGX_SUCCESS)
    {
        if (retstatus == -1)
            return se_ret;
    ...
    if (0 != memcmp(msg_hash,
                    &(((sgx_quote3_t*)msg3->msg3_body.quote_buffer)->report_body.report_data),
                    sizeof(msg_hash)))
    {
        pcd_log_error("ERROR: ECDH message 3: SHA256(GB || GA) mismatch\n");
        return SGX_ERROR_MAC_MISMATCH;
    }
...
// Function pcd_dh_dcap_responder_gen_msg1 generates M1 message and makes update to the context of the session.
sgx_status_t pcd_dh_dcap_responder_gen_msg1(pcd_dh_dcap_msg1_t* msg1, sgx_dh_session_t* sgx_dh_session)
{
    sgx_status_t se_ret;
    sgx_internal_dh_session_t* session = (sgx_internal_dh_session_t*)sgx_dh_session;
                                         ...
                                         pcd_dh_dcap_msg3_t *msg3)
{
    ...
    memset(msg3, 0, sizeof(pcd_dh_dcap_msg3_t));
    ...
    // Verify message 3 report
    se_ret = ocall_ecdsa_quote_verification(&retstatus, msg3->msg3_body.quote_buffer, quote_size);
...
//pcd_dh_dcap_responder_proc_msg2 processes M2 message, generates M3 message, and returns the session key AEK.
sgx_status_t pcd_dh_dcap_responder_proc_msg2(const pcd_dh_dcap_msg2_t* msg2,
                                        pcd_dh_dcap_msg3_t* msg3,
                                        ...
                                        sgx_key_128bit_t* aek)
{
    ...
    // TODO Verify msg2 quote!

    maced_size = static_cast<uint32_t>(sizeof(pcd_dh_dcap_msg3_body_t));

    //Verify the MAC of message 3 obtained from the Session Responder
    se_ret = verify_cmac128((const uint8_t*)dh_smk, (const uint8_t*)&msg2->msg2_body, maced_size, msg2->cmac);
    if(SGX_SUCCESS != se_ret)
    {
        return se_ret;
    }

    memcpy(quote, &msg2->msg2_body.quote_buffer, sizeof(msg2->msg2_body.quote_buffer));

    // Verify message 2 report
    uint32_t retstatus;
    quote_size = msg2->msg2_body.quote_size;
	se_ret = ocall_ecdsa_quote_verification(&retstatus, quote, quote_size);
    if (se_ret == SGX_SUCCESS)
    {
        if (retstatus == -1)
    ...
    // Verify message 2 report data
    if (0 != memcmp(msg_hash,
                    &(((sgx_quote3_t*)msg2->msg2_body.quote_buffer)->report_body.report_data),
                    sizeof(msg_hash)))
    {
        pcd_log_error("ERROR: ECDH message 2: SHA256(GB || GA) mismatch\n");
        return SGX_ERROR_MAC_MISMATCH;
    }
