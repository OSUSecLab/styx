...
typedef struct _pcd_dh_dcap_msg1_t
{
    sgx_ec256_public_t  g_a;     /* the Endian-ness of Ga is Little-Endian */
} __attribute__((packed)) pcd_dh_dcap_msg1_t;

typedef struct _pcd_dh_dcap_msg3_body_t
{
	uint32_t     quote_size;
    uint8_t quote_buffer[SGX_QUOTE3_BUFFER_SIZE];
} __attribute__((packed)) pcd_dh_dcap_msg3_body_t;

typedef struct _pcd_dh_dcap_msg2_t
{
    sgx_ec256_public_t  g_b;     /* the Endian-ness of Gb is Little-Endian */
    uint8_t            cmac[SGX_DH_MAC_SIZE];
    pcd_dh_dcap_msg3_body_t msg2_body;
} __attribute__((packed)) pcd_dh_dcap_msg2_t;

typedef struct _pcd_dh_dcap_msg3_t
{
    uint8_t            cmac[SGX_DH_MAC_SIZE];
    pcd_dh_dcap_msg3_body_t msg3_body;
} __attribute__((packed)) pcd_dh_dcap_msg3_t;
...
} __attribute__((packed)) sgx_dh_session_enclave_identity_t;
