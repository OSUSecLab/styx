...
extern "C" int pcd_secret_register(pcd_identity_t *id, pcd_secret_t *input_secret) {
	...
	if (ret.second == false) {
		...
		pcd_log("INFO: 0x%016llX's secret already exists\n", ((uint64_t *)id)[0]);
		return PCD_REINIT;
	}
	...
	pcd_secret_mutex.unlock();
	pcd_log("INFO: 0x%016llX's secret registered\n", ((uint64_t *)id)[0]);
	return PCD_OK;
}
...
extern "C" int pcd_secret_retrieve(pcd_identity_t *id, pcd_secret_t **output_secret) {
	...
	if(it == pcd_secret_store.end()) {
		pcd_secret_mutex.unlock();
		pcd_log("INFO: 0x%016llX's secret not found\n", ((uint64_t *)id)[0]);
		return PCD_NOT_FOUND;
	}

	pcd_log("INFO: 0x%016llX's secret found\n", ((uint64_t *)id)[0]);
